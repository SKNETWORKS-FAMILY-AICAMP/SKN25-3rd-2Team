...
이 문서는 ArXplore 저장소에서 AI 도구가 작업할 때 따라야 하는 공통 규칙이다. 현재 저장소는 이미 수집, 파싱, 적재, 임베딩 기반을 갖춘 상태이며, AI는 이 기반을 임의로 다시 설계하는 것이 아니라 그 위에서 retrieval, answer chain, prompt, topic document, UI 계층을 다듬는 방식으로 작업해야 한다.
...
중요한 점은, 더 이상 "1번 역할이 PostgreSQL 쓰기 경로와 최소 retrieval까지 먼저 완성해야 한다"는 초기 전제를 기준으로 해석하면 안 된다는 것이다. 현재 데이터 파이프라인과 적재 구조는 이미 준비되어 있으며, AI는 이 기반 위의 상위 계층과 계약 안정성에 초점을 맞춰야 한다.
...
즉, 현재 구조를 "서버 Airflow가 모든 단계를 직접 실행하는 구조"로 오해하면 안 된다. 수집 자동화와 무거운 파싱/임베딩 실행은 분리돼 있다.

## 3. 절대 임의 변경하면 안 되는 것

### `TopicDocument` 계약

`src/core/models.py`의 `TopicDocument`, `PaperRef`, `RelatedTopic`는 팀 공용 데이터 계약이다. 이 모델은 생성 체인의 출력이자 저장 구조이자 UI 입력이다. 한 계층의 편의를 위해 필드를 바꾸면 다른 계층이 동시에 깨진다.
...
- `prepare_jobs`, `prepare-worker`, parser runtime 같은 현재 운영 핵심을 무시하고 초기 설계 문맥으로 구조를 되돌리는 것
...
| 표준 표현 | 설명 |
|---|---|
| `토픽 문서` | `TopicDocument` 단위 구조화 문서 |
| `개요` | topic 수준의 흐름 요약 |
| `핵심 발견` | 토픽 수준의 공통 기여와 차이 |
...
문맥에 맞는다는 이유로 다른 용어를 섞어 쓰지 않는다. 문서, 코드, 프롬프트 사이 용어가 어긋나면 유지보수가 급격히 어려워진다.
...
- retrieval, answer, prompt, topic document, UI 경계를 침범하는 변경은 더 보수적으로 다룬다
...
3. 한국어 번역과 상세 요약 규칙
4. topic document 생성 품질
5. UI 소비 계층 연결

AI는 더 이상 "파이프라인 뼈대를 새로 만드는 작업"을 기본값으로 두지 않는다. 이미 구현된 `prepare_jobs`, `prepare-worker`, 로컬 parser runtime을 이해한 뒤, 그 위에 있는 계층을 개선하는 것이 현재 기준이다.
