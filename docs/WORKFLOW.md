...
- `arxplore_backfill_collect_papers`: 하루에 최대 30일씩 과거 HF Daily Papers 원본을 MongoDB에 백필한다. 진행 상태는 MongoDB pipeline state에 cursor로 저장한다.
- `arxplore_enrich_papers_metadata`: PostgreSQL에 저장된 논문 중 arXiv 메타데이터가 비어 있는 항목을 후속 보강한다.
...
`target_date`를 비우면 오늘 날짜를 사용하고, `max_papers`를 비우면 해당 날짜 raw payload 전체를 대상으로 prepare를 수행한다. `backfill_collect_papers`는 `cursor_date`를 비우면 저장된 MongoDB pipeline state cursor를 사용하고, `oldest_date`에 도달할 때까지 하루 최대 `batch_days`일씩 거슬러 올라간다. `enrich_papers_metadata`는 `max_papers`만 받아 미보강 논문을 일부씩 채운다.
...
2. `arxplore_backfill_collect_papers`로 하루 최대 30일씩 과거 raw를 누적한다.
