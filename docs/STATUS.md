...
이 문서는 2026-04-04 기준으로 프로젝트 전반의 구현 상태를 정리한 것이다. 팀원이 자기 담당 영역의 현재 상태와 의존 관계를 빠르게 파악하는 것이 목적이다.
...
| 한국어 번역·상세 요약 | 개별 논문 번역/요약 프롬프트 없음. 현재 프롬프트는 토픽 수준만 |
| hybrid retrieval | lexical + vector 결합 로직 없음 (각각 독립 동작) |
...
A, B와 의존 관계 없이 프롬프트 설계와 품질 검증을 먼저 진행할 수 있다.
